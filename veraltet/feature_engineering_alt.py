import numpy as np
import pandas as pd
from veraltet.utils_alt import safe_location, get_match_phase, safe_eval, get_score_status, get_movement_angle, count_opponents_nearby, get_action_cat, event_success, count_free_teammates
...
def apply_feature_engineering(df):
    ...
    # TODO: add contextual features like: 
    ## letzte N Events (als One-Hot-Sequenz oder Embedding),
    # Zeit seit letztem Event (in Sekunden) (evtl. auch Events nicht mehr aufeinenanderfolgend zählen)
    # Länge der aktuellen Event-Sequenz (Anzahl Aktionen im Ballbesitz)
    # Fortschritt Richtung Tor (Netto-Distanzgewinn seit Ballbesitzstart)
    # Wiederholte Muster (z. B. Pass-Pass-Dribble → häufig vor Flanken)
    # Spielerrollen der letzten Aktionen (z. B. Innenverteidiger am Ball → geringe Schusswahrscheinlichkeit)
    # contextual features
