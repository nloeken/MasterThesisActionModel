import numpy as np
import pandas as pd
from utils import safe_location, get_match_phase, safe_eval, get_score_status, get_movement_angle, count_opponents_nearby, get_action_cat, event_success, count_free_teammates, get_time_since_last_event, get_progress_to_goal, assign_zone
...
def apply_feature_engineering(df):
    ...
    # positions relative to the pitch
    ...
    df['distance_to_goal'] = np.sqrt((x_goal - df['x'])**2 + (y_goal - df['y'])**2).round(2)
    # feature 5: angle to goal (in radians)
    df['angle_to_goal'] = np.arctan2(y_goal - df['y'], x_goal - df['x']).round(2)
    # feature 3: zone (0-19)
